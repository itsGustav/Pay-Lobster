...
exports.CommissionSplitter = void 0;
 ...
 * Commission Splitter
 */
class CommissionSplitter {
    constructor(dataDir = DATA_DIR) {
        this.splitsPath = path_1.default.join(dataDir, 'commission-splits.json');
        this.templatesPath = path_1.default.join(dataDir, 'split-templates.json');
    }
    async loadSplits() {
        try {
            const data = await promises_1.default.readFile(this.splitsPath, 'utf-8');
    ...
    async saveSplits(splits) {
        await promises_1.default.mkdir(path_1.default.dirname(this.splitsPath), { recursive: true });
        await promises_1.default.writeFile(this.splitsPath, JSON.stringify(splits, null, 2));
    }
    async loadTemplates() {
        try {
            const data = await promises_1.default.readFile(this.templatesPath, 'utf-8');
            return JSON.parse(data);
        }
        catch {
            return [];
    ...
    async saveTemplates(templates) {
        await promises_1.default.mkdir(path_1.default.dirname(this.templatesPath), { recursive: true });
        await promises_1.default.writeFile(this.templatesPath, JSON.stringify(templates, null, 2));
    }
    // ============ Split Creation ============
    /**
    ...
    async createSplit(params) {
        ...
        const validation = this.validateSplits(params.totalCommission, params.recipients);
        if (!validation.valid) {
    ...
    async createFromTemplate(templateId, params) {
        ...
        const recipients = template.recipients.map(tr => {
     ...
     */
    calculatePayouts(split) {
        const total = parseFloat(split.totalCommission);
        const payouts = [];
        let remaining = total;
        // Sort by tier (lower tier = paid first)
        const sortedRecipients = [...split.recipients].sort((a, b) => (a.tier || 0) - (b.tier || 0));
     ...
     * Validate splits add up correctly
     */
    validateSplits(total, recipients) {
     ...
     */
    async retryFailed(splitId, sendFn) {
        const splits = await this.loadSplits();
        const split = splits.find(s => s.id === splitId);
        if (!split)
            return null;
    ...
    async list(filters) {
        ...
        if (filters?.recipientAddress) {
            splits = splits.filter(s => s.recipients.some(r => r.walletAddress.toLowerCase() === filters.recipientAddress.toLowerCase()));
        }
        if (filters?.fromDate) {
        ...
        return splits.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    }
    /**
...
exports.CommissionSplitter = CommissionSplitter;
exports.default = CommissionSplitter;
